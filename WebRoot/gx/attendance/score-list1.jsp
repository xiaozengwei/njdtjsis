...
%>
<script>
    function opNavtab11(obj,userId,start,end,userName){
        var name=encodeURIComponent(encodeURIComponent(encodeURIComponent(userName)));
        $(obj).navtab({id:'mynavtab', url:'<%=basePath%>score/get-score-detail1.do?filter_EQS_userId='+userId+'&filter_GED_recordTime='+start+'&filter_LED_recordTime='+end+'&name='+name,title:$(obj).data("title")});
    }
</script>
        ...
        <table class="table table-bordered table-hover table-striped table-top" data-selected-multi="true">
            ...
            <c:forEach items="${page.result}" var="score" varStatus="status">
                <tr data-id="">
                    ...
                    <td align="center">
                        <button data-toggle="navtab" type="button" class="btn-green" data-icon="edit" data-title="积分详情"
                                onclick="opNavtab11(this,'${score.userId}','<fmt:formatDate value="${score.scoreStart}" pattern="yyyy-MM-dd HH:mm:ss"/>','<fmt:formatDate value="${score.scoreEnd}" pattern="yyyy-MM-dd HH:mm:ss"/>','${score.userName}')" >编辑</button>
                    </td>
                </tr>
            </c:forEach>
            </tbody>

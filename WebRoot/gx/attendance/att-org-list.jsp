...
<script type="text/javascript">
    function opNavtab11(obj,userId,time){
        $(obj).navtab({id:'mynavtab', url:'<%=basePath%>attendance/att-detail.do?userId='+userId+'&time='+time,title:$(obj).data("title")});
    }
</script>
<div class="bjui-pageHeader">
    <form id="pagerForm" data-toggle="ajaxsearch" action="<%=basePath%>attendance/att-list-test.do?level=${level}" method="post" style="margin-bottom: 0">
        ...
        <div class="bjui-searchBar">
            <br/>
            <label>部门：</label>
            <input tepe="test" id="orgName" name="filter_LIKES_orgName" value="${param.filter_LIKES_orgName}" class="form-control" size="10">
            ...
            <label>姓名：</label>
            <input tepe="test" id="userName" name="filter_LIKES_userName" value="${param.filter_LIKES_userName}" class="form-control" size="10">
            <label>开始日期：</label>
...
<%--                <label>开始日期：</label>--%>
<%--                <input type="text" name="start" data-toggle="datepicker" data-rule="date" size="19">&nbsp;&nbsp;--%>
<%--                <label>结束日期：</label>--%>
<%--                <input type="text" name="end" id="j_custom_issuedate" data-toggle="datepicker" data-rule="date" size="19">--%>
<%--                <label>类型：</label>--%>
<%--                <select name="type" data-toggle="selectpicker" id="on">--%>
<%--                    <option value="">全部</option>--%>
<%--                    <option value="0" ${param.filter_LIKES_resultType eq '早退'?'selected':''}>监理</option>--%>
<%--                    <option value="1" ${param.filter_LIKES_resultType eq '正常'?'selected':''}>领导</option>--%>
<%--                    <option value="2" ${param.filter_LIKES_resultType eq '迟到'?'selected':''}>管理人员</option>--%>
<%--                    <option value="3" ${param.filter_LIKES_resultType eq '早退'?'selected':''}>工人</option>--%>
<%--                </select>--%>
<%--                <button type="button" class="btn-green" data-url="<%=basePath%>attendance/export.do" data-toggle="doexport" data-confirm-msg="确定要导出吗？" data-idname="delids" data-group="ids">导出</button>--%>
<%--            </div>--%>
        </div>
    </form>
</div>
        ...
        <c:forEach items="${page.result}" var="att" varStatus="status">
            <tr data-id="6">
                ...
                <td><button data-toggle="navtab" type="button" class="btn-default" style=""  data-title="查看详情"
                            onclick="opNavtab11(this,'${att.userId}','${att.signTime}')">查看</button>
                </td>
            </tr>
        </c:forEach>
        </tbody>

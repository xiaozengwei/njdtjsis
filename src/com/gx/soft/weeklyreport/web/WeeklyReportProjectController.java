 ...
 */
@Controller
@RequestMapping("weeklyreportproject")
@SessionAttributes("user_session")
public class WeeklyReportProjectController {
    ...
    public String List(@ModelAttribute Page page, @RequestParam Map<String, Object> parameterMap, Model model,@ModelAttribute("user_session") VUser user){
        ...
        page = weeklyReportProjectManager.pagedQuery(page, propertyFilters);
        model.addAttribute("page", page);
        return "weeklyreport/project/weeklyreport-list";
    }
    @RequestMapping("weeklyreport-input")
    public String input(@RequestParam(value = "rowId", required = false) String rowId, Model model) {
        WeeklyReportProject weeklyReportProject;
        if (rowId != null) {
            ...
            weeklyReportProject = new WeeklyReportProject();
        }
        model.addAttribute("weeklyReportProject", weeklyReportProject);
        return "weeklyreport/project/weeklyreport-input";
    }
    ...
    @ResponseBody
    public Map<String, Object> save(WeeklyReportProject weeklyReportProject, @ModelAttribute("user_session") VUser user) {
        Map<String, Object> resMap = new HashMap<String, Object>();
        String statusCode = "200", message = "操作成功";
        Timestamp ts = DateUtil.getDate();
        try {
            WeeklyReportProject dest ;
            String id = weeklyReportProject.getRowId();
            if (id != null && id.length() > 0) {
                dest = weeklyReportProjectManager.get(id);
                String rowID=id;
                ...
                beanMapper.copy(weeklyReportProject, dest);
            } else {
                weeklyReportProject.setRowId(null);
                dest = weeklyReportProject;
                dest.setCreateTime(ts);
            }
            dest.setOtherWork(dest.getOtherWork().trim());
            dest.setThisWeekWork(dest.getThisWeekWork().trim());
            weeklyReportProjectManager.save(dest);
    ...
    public Map<String, Object> viewRemove(String delids) {
        ...
        try {
            if (delids != null && delids.length() > 0) {
                ...
                for (String rowId : ids) {
                    if (rowId != null && rowId.length() > 0) {
                        weeklyReportProjectManager.removeById(rowId);
                    }
                }
            }
        } catch (Exception e) {
...
//        String bol="yes";
        ...
        try {
            if (!StringUtils.isEmpty(fileOriginalName)) {
                ...
                mFilePath = mFilePath.substring(0, 2) + "\\njdtjsis" + "\\weeklyreportproject\\" + decodeFileName;
...
//            bol="no";
    ...
    public String edit(@RequestParam(value = "rowId", required = false) String rowId, Model model) {
        ...
        model.addAttribute("weeklyReportProject", weeklyReportProject);
        model.addAttribute("fileRecordList", fileRecordList);
        return "weeklyreport/project/weeklyReport-look";
    }
}
